...
below is verbatim, newest first, including every past "Decisions" list
(still append-only: never edit an entry here, only add new history to
the TOP of this file when the head file is trimmed again).
...
Part 1 (the adapter seam) shipped in graph-quest earlier tonight (gq-v31); this is
the blipwork half, built exactly per `graph-quest\MIGRATION-PLAN.md` (her four calls,
none re-asked) + `FUNFUN-PART2-BRIEF.md` (the build brief, committed). Agent spend
≈ 0.49M Opus (S1 plumbing 0.22M · S2 screens 0.27M), under the 0.6–0.8M estimate.

**What a learner sees:** in the Functions chapter, a 📈 Fun Functions strip with the
15 quest tiles in map order (grandfathered unlocks ported; ✓ + best-% per tile).
Tapping a tile plays that quest — climb, sweep, paint, drag — inside blipwork, in
English, semicircles OFF, then a results card (📈, %, score, +XP/+💎) and the tile
updates. Dashboard: one `📈 N` class-plays chip on the Functions row.

**The engineering shape (for the next session):**
- `js/funfun/` = SYNCED COPY of graph-quest (generated by
  `graph-quest/tools/sync-to-blipwork.py`, 35 files + manifest — NEVER hand-edit;
  fixes go in graph-quest, then re-sync).
- Isolation = a SHADOW ROOT (`js/funfun-play.js` `funfunShadow()`): 60 class names
  collide between the two apps (measured), so Fun Functions lives in its own sealed
  DOM with its stylesheet linked inside. Host overrides in the shadow: `.view`
  min-height 0 (seam fact a) and the mount's own `‹ Map` link hidden (foreman call —
  blipwork's back bar does the same teardown; the standalone keeps its link).
- Server side: `supabase/migration-funfun.sql` — table `funfun_progress` + four RPCs
  (`mhq_funfun_state/met`, `mhq_submit_funfun`, `mhq_admin_funfun`). ⚠️ deliberately
  NO rewrite of `mhq_get_state`/`mhq_admin_data` (the copy-forward danger). XP is
  recomputed server-side from the `answered` outcome record on the static-round
  economy (`_mhq_dice_xp`), clamp 1000 → 25% on replays-after-done, pass bar 0.7
  (graph-quest's, NOT blipwork's 0.8 — the bar the learner actually played against),
  flat 10 💎. Client never names an amount.
- `?local=1` mirror in `js/local-backend.js` (LS key `mhq.funfun`); kill switch
  `FUNFUN_ENABLED` in config.js; results show SA decimal comma (`3,5 / 6`).

**MIGRATION ✅ APPLIED TO LIVE by Claude (her "yes, you can run it"), then
`/migration-check`-verified:** 4 RPCs auth-gated + granted, 2 internals revoked,
RLS on, anon can't touch the table, and learner rows byte-identical before/after
(students/progress/blips md5 hashes + total XP 9980, 22 students).

**Live-verified after push:** Pages built `f0768f8`, sw **mhq-v65** served, funfun
files 200, review fixes present in the live JS, and a real quest mounted+playing on
the live site under `?local=1` (offline backend — no learner rows touched). One
transient CDN 503 on an unchanged file during propagation; clean on reload.

**Foreman review notes (both sessions' work verified fresh, not taken on report):**
harnesses `verify-funfun.html` 215 · `verify-funfun-backend.html` 160 · dice 134 ·
func 75 · store ~4030 all green in the foreman's own browser; the play loop walked
at 375px. ⚠️ Local-testing trap hit AGAIN tonight: python's bare http.server sends
no Cache-Control, so the browser serves heuristically-cached STALE modules without
even revalidating — `fetch(url, {cache:"reload"})` per file, then reload, is the
cure. Live GitHub Pages sends real headers; learners are not exposed.

### ⏳ Pending on Megan
- 📱 5 min **[whenever]**: close-and-reopen the PWA twice (sw v65) → Functions →
  📈 tile 1 (Discover) → play it to the end → results card pays → tile shows ✓/%.
- 📱 (carried) per chapter: one 🎲 round each in Finance, Number Patterns, 2D Trig.

## 📌 Decisions (append-only, 2026-08-23 night)
- **Shadow DOM is the mount isolation** — not a curated CSS reset list (60
  collisions measured; a list would rot). `funfunShadow()` is the one shadow setup;
  verify-funfun.html imports IT, never a copy.
- **Fun Functions pass bar inside blipwork = 0.7** (graph-quest's own), while static
  rounds keep 0.8 — two bars, deliberate, documented in the migration header.
- **`mhq_get_state`/`mhq_admin_data` untouched** — funfun gets its own four RPCs.
- **The mount's `‹ Map` link is hidden host-side** (there is no map in blipwork;
  one back affordance, blipwork's own). Standalone unaffected.
- **Half-credit scores print with the SA decimal comma** (`3,5 / 6`) — blipwork's
  own content convention, wins over "English decimal point".
- **`sessions/funfun-walk/` is gitignored** — walk screenshots stay out of the
  public repo, same rule as tools/shots.

## 🎲 (earlier that evening) DICE ×3 SHIPPED — Finance, Number Patterns, 2D Trig dice LIVE on sw v64, + the two Euclid cosmetics + four review fixes
...
- ~~📈 Fun Functions MOUNT Part 2~~ ✅ SHIPPED 2026-08-23 night (sw v65, commit
  `f0768f8`, migration applied) — see the entry at the top of this file. Future Fun
  Functions fixes: edit graph-quest, run `graph-quest/tools/sync-to-blipwork.py`,
  re-ship here (SW bump). Later, not blocking: unify the two function-graph engines.

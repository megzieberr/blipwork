/* ============================================================
   BLIPWORK · EXAM FOCUS — circle diagram engine
   ------------------------------------------------------------
   PORTED VERBATIM from circle-geometry-game/js/engine.js on
   2026-08-22 (EXAM-FOCUS-PLAN.md build order step 3, her design:
   "port that one file into Blipwork"). Circle Quest itself was NOT
   touched — it keeps its own copy and stays the owner of circle-geo
   teaching and drill rounds; this copy exists so Euclidean EXAM
   ...
   in-place change, clearly marked where it sits:
     ...
     · angleSVG()'s o.mark branch now understands o.mark === "square"
       ...
       Every other truthy o.mark value still draws the chevron, byte
       for byte, so every Circle Quest diagram this file could ever be
       handed renders identically. (Port-day gap 1, recorded in
       js/exam/_pending-engine-port/README-PENDING.md: "o.mark draws a
       chevron, not a right-angle square. Q4(b) needs a square at M …
       do not silently ship a chevron for a 90° angle" — Grade 11
   ...
   hit points come across with the port, unused, so it stays an
   addition later rather than a rebuild. Do NOT build it this year.
   ============================================================ */
...
function angleSVG(cx, cy, from, to, text, o, accent, W, H) {
  ...
  if (o.mark === "square") {
    /* BLIPWORK-ONLY, ADDITIVE (2026-08-22, port-day gap 1). A RIGHT-ANGLE
       SQUARE: step m along each leg, and the fourth corner of the square is
       the vector sum of those two steps from the vertex. Built from the two
       real leg directions rather than from an assumed 90°, so if the angle
       ever stopped being a right angle the little box would visibly stop
       being a square — the same "the picture cannot lie" discipline the
       rest of this engine runs on. Opt-in on the exact string "square";
       every other truthy o.mark still takes the chevron branch below,
       unchanged. */
    const m = 11;
    const [ax, ay] = pol(cx, cy, m, from);
    const [bx, by] = pol(cx, cy, m, from + s);
    const kx = ax + bx - cx, ky = ay + by - cy;
    out += `<path d="M ${N(ax)} ${N(ay)} L ${N(kx)} ${N(ky)} L ${N(bx)} ${N(by)}" fill="none" stroke="${col}" stroke-width="2"/>`;
  } else if (o.mark) {
    const m = 12;
...
/* ============================================================
   ...
   lights the wedge on angle A; 'prove ABCD is cyclic' lights the four
   sides." The engine already ships the two marker-pen primitives that
   needs (o.hl on an angle, c.hl on a chord); what was missing was a
   small, checkable way for a QUESTION MODULE to say which ones to
   light for a given sub-part, without every module hand-building a
   whole second spec. highlightedSpec() is that: base spec in, base
   spec + this part's marker-pen strokes out. The base spec is never
   mutated, so the same spec object is reused across every part.

   A HIGHLIGHT SET (the shape js/exam/_schema.js validates and
   js/exam-play.js renders):
     {
       bare:   true,          // draw the figure WITHOUT its own angle
                              // labels — the "bare-figure rule": a
                              // question diagram must not hand over
                              // the labelling the proof is asking for
       angles: [ { at:"A", legs:["O","tg-"], v:90, t:"", o:{mark:"square"} } ],
       chords: [ ["O","A"], ["A","T"], ["P","M","t1"] ],
     }
   A chord ref is [a, b] or [a, b, mark], where `mark` is the engine's
   own chord-mark code ("t1"/"t2"/"t3" equal-length ticks, "p1"/"p2"
   parallel arrows). The mark matters where colour alone cannot carry
   the message: highlighting the two halves of a bisected chord looks
   EXACTLY like highlighting the whole chord — one amber stroke either
   way — so "bisects" only becomes visible when each half also gets its
   equal-length tick. (Found by looking at the rendered PNG, 2026-08-22;
   the DOM check had passed happily.)
   Every angle ref carries `v`, its TRUE value — so a highlight is
   verified by verifyDiagram() exactly like an authored angle is, and
   a wedge drawn on the wrong side of a leg fails the harness instead
   of quietly shipping. Chord refs may name a drawn chord OR one of
   the tangent segments an `ext` point creates; the tangent case is
   emitted as a duplicated chord pair, which lands the translucent
   marker stroke under the same line the engine already drew.
   ============================================================ */
...
/* Every segment this spec actually DRAWS, as {p1,p2} coordinate pairs:
   ...
   nothing to highlight BETWEEN. */
function drawnSegments(g) {
  const segs = g.chordSegs.map(c => [c.p1, c.p2]);
  g.extTangents.forEach(s => segs.push([s.p1, s.p2]));
  return segs;
}

/* Is the segment a–b one the figure draws, or a PIECE of one? The piece
   case is what makes "highlight the two halves of chord PQ" (P–M, M–Q)
   and "highlight only M→T along the radius O→T" legal: both endpoints
   lie on a drawn segment, so the marker stroke lands on ink that is
   really there. A pair that floats free of every drawn line is rejected
   — highlighting a line the learner cannot see would be a lie about the
   figure, which is the one thing this engine exists to prevent. */
function segmentIsDrawn(g, a, b) {
  const P = g.pts[a], Q = g.pts[b];
  if (!P || !Q) return false;
  if (Math.hypot(P.x - Q.x, P.y - Q.y) < 0.5) return false;      // a zero-length "segment"
  return drawnSegments(g).some(([s1, s2]) =>
    segDist(P.x, P.y, s1.x, s1.y, s2.x, s2.y) < 0.5 &&
    segDist(Q.x, Q.y, s1.x, s1.y, s2.x, s2.y) < 0.5);
}

/* Return a NEW spec with this part's marker-pen highlights applied.
   `d` is never mutated. */
export function highlightedSpec(d, hl) {
  ...
  (hl.chords || []).forEach(([a, b, mk]) => markFor.set(pairKey(a, b), mk || null));
  const covered = new Set();
  out.chords = (d.chords || []).map(c => {
    const a = Array.isArray(c) ? c[0] : c.a, b = Array.isArray(c) ? c[1] : c.b;
    const k = pairKey(a, b);
    if (!markFor.has(k)) return c;
    covered.add(k);
    const mk = markFor.get(k) || (Array.isArray(c) ? null : c.mk) || null;
    return Array.isArray(c) ? { a, b, mk, hl: HL } : { ...c, mk, hl: HL };
  });
  /* a highlighted TANGENT SEGMENT (from an ext point), or a PIECE of a
     drawn chord, is not itself in d.chords — add it as a duplicated pair
     so the marker stroke (and any tick) lands on the line that is already
     drawn there. */
  (hl.chords || []).forEach(([a, b, mk]) => {
    if (covered.has(pairKey(a, b))) return;
    covered.add(pairKey(a, b));
    out.chords = out.chords.concat([{ a, b, mk: mk || null, hl: HL }]);
  });

  const base = hl.bare ? [] : (d.angles || []);
  out.angles = base.concat((hl.angles || []).map(a => ({
    at: a.at, legs: a.legs, t: a.t || "",
    o: { ...(a.o || {}), v: a.v, hl: a.hlColour || HL },
  })));
  return out;
}

/* Structural check for a highlight set against its spec: every name it
   references must exist in the resolved geometry, and every chord ref
   must be a segment the engine actually draws. Returns a list of
   human-readable issues (empty === clean). Used by js/exam/_schema.js;
   the numeric side (does the wedge really measure v?) is verifyDiagram's
   job, run on the highlighted spec. */
export function diagramRefIssues(d, hl, label) {
  ...
  (hl.angles || []).forEach((a, i) => {
    ...
    if (!a || !names.has(a.at)) { issues.push(`${at}: vertex "${a && a.at}" is not a point in this spec`); return; }
    if (!Array.isArray(a.legs) || a.legs.length !== 2) { issues.push(`${at}: legs must be a pair`); return; }
    a.legs.forEach(leg => {
      if (leg === "tg+" || leg === "tg-") {
        if (!g.pts[a.at].circ) issues.push(`${at}: leg "${leg}" needs the vertex to sit ON the circle`);
        return;
      }
      if (!names.has(leg)) issues.push(`${at}: leg "${leg}" is not a point in this spec`);
    });
    if (typeof a.v !== "number") issues.push(`${at}: highlight must declare its true value v (a number) so verifyDiagram can check it`);
  });

  (hl.chords || []).forEach((c, i) => {
    const at = `${label}.chords[${i}]`;
    if (!Array.isArray(c) || c.length < 2 || c.length > 3) { issues.push(`${at}: must be [a,b] or [a,b,mark]`); return; }
    const [a, b, mk] = c;
    if (mk !== undefined && !/^[tp][1-3]$/.test(String(mk))) issues.push(`${at}: chord mark "${mk}" must be t1/t2/t3 (equal ticks) or p1/p2 (parallel arrows)`);
    if (!names.has(a)) issues.push(`${at}: "${a}" is not a point in this spec`);
    if (!names.has(b)) issues.push(`${at}: "${b}" is not a point in this spec`);
    if (names.has(a) && names.has(b) && !segmentIsDrawn(g, a, b)) {
      issues.push(`${at}: ${a}${b} is not a segment this spec draws, nor a piece of one`);
    }
  });
